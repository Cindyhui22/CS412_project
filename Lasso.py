...
# Set up the grid search for standardized data with return_train_score=True
grid_lasso_std = GridSearchCV(lasso, param_grid_lasso, cv=5, return_train_score=True)

# Fit the model for standardized data
grid_lasso_std.fit(X_train_std, y_train.ravel())  
...
# Make predictions for standardized data on the test set using the best estimator
y_pred_lasso_std = grid_lasso_std.best_estimator_.predict(X_test_std)
# here y_pred_lasso_std means the X_test_std is used to predict y_test; y_pred itself is not standardized

# Calculate Mean Squared Error for standardized data on the test set
...
plot_validation_curve(grid_lasso_std).savefig(f'{model_name}_{data_size}_validation_curve.png', dpi=300, bbox_inches='tight')
plot_learning_curve(grid_lasso_std.best_estimator_, X_train_std, y_train).savefig(f'{model_name}_{data_size}_learning_curve.png', dpi=300, bbox_inches='tight')
plot_residuals(y_test, y_pred_lasso_std).savefig(f'{model_name}_{data_size}_residuals.png', dpi=300, bbox_inches='tight')
plot_predicted_vs_actual(y_test, y_pred_lasso_std).savefig(f'{model_name}_{data_size}_predicted_vs_actual.png', dpi=300, bbox_inches='tight')
